...
import { authenticateToken, authorizeRoles, type AuthenticatedRequest } from '../middlewares'
...
export const characterRouter = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
  ...
  try {
    ...
    if (url == '/chacacters' && method == HttpMethod.DELETE) {
      const isAuthorized = await authorizeRoles(Role.ADMIN, Role.USER)(
        req as AuthenticatedRequest,
        res
      )
      ...
      const succes = deleteCharacters(characterId)
      if (!succes) {
        res.statusCode = 404
        res.end(JSON.stringify({ message: 'Character not found' }))
        return
